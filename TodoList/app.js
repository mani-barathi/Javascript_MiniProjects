...
function createTodo(todo) {
    ...
    if (todo.completed)
        row.classList.add('completed-div');

    row.innerHTML = `
        <p class="${(todo.completed) ? 'completed-text' : ''}">${todo.text}</p>
        <div class="todo-btns">
            <button data-id=${todo.id} onClick='updateTodo(this)' class="tick"> <i class="fas fa-check"></i></button>
            <button data-id=${todo.id} onClick='deleteTodo(this)' class="trash"> <i class="fas fa-trash"></i></button>
        </div>`;
...
function deleteTodo(element) {
    ...
    for (i in todos) {      // finding todo using id
        if (todos[i].id == id) {
            todos.splice(i, 1)
            break;
        }
    ...
    parentDiv.addEventListener('transitionend', () => {  // for the fading to todo 
        parentDiv.remove();
    });
}
...
function fetchAllTodos() {
    ...
    todos = (temp) ? JSON.parse(temp) : [{ id: 1606813611717, text: "This is a Task!", completed: false },
    { id: 1606813625543, text: "This is a completed Task!", completed: true }];
