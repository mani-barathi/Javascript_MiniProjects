...
function resetErrors() {
    ...
    for (let el of ['name-feedback', 'city-feedback', 'email-feedback', 'language-feedback'])
...
function handleFormSubit(event) {
    ...
    if (form.name.value.length < 3) {
        ...
        document.getElementById('name-feedback').textContent = 'Name should atleast be 3 character'
        isCorrect = false
    }
    ...
    if (cities.includes(form.city.value.toLowerCase()) === false) {
        ...
        document.getElementById('city-feedback').textContent = 'Select a valid City from list'
        isCorrect = false
    }

    if (form.language.value.toLowerCase() === 'click to see the options') {
        form.language.classList = 'form-control is-invalid'
        document.getElementById('language-feedback').textContent = 'Select your main programming Language'
        isCorrect = false
    }
