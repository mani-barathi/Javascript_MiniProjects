...
itemContainers.forEach(itemContainer => {
    itemContainer.addEventListener('dragover', (e) => {
        ...
        if (itemContainer === favorites && isFavouritesFull())
            return
        ...
        if (!afterElement)                      //  no element append dragging item to container
            itemContainer.appendChild(draggingItem)
        else                                    // insert dragging item after the particular item in container
...
function getDragAfterElement(itemContainer, mousePositionY) {
    ...
    }, { offset: Number.NEGATIVE_INFINITY })  // initial closest
