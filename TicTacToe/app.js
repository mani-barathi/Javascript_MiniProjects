...
function showGameOverModal(isDraw, winner = null) {
    if (isDraw)
        gameOverModalText.textContent = 'Game DRAW!';
    else
        gameOverModalText.textContent = `${winner} won!`;
...
function checkWin(board = null) {
    ...
    let diag1 = board[0] === board[4] && board[4] === board[8] && board[8] !== '-';
    let diag2 = board[2] === board[4] && board[4] === board[6] && board[6] !== '-';

    let possibilities = [row1, row2, row3, col1, col2, col3, diag1, diag2];

    for (let i = 0; i < possibilities.length; i++) {
        if (possibilities[i])       // checking if any one possiblity is true, 
...
function checkDraw() {
    if (!foundWinner && count == 8)
        return true;
    return false;
}

function getComputerMove() {
    let possibileMoves = arr.map((el, i) => el === '-' ? i : el)
        .filter(a => (a >= 0 && a <= 8))
    ...
    let randomMove = possibileMoves[Math.floor(Math.random() * possibileMoves.length)];
    return randomMove;
}
...
function handleMoveOnComputerVsPlayer(event, isComputer) {
    ...
    if (checkWin()) {
        ...
        textToPlay.textContent = `${(isComputer) ? 'X' : 'O'} to Play`;
...
function handleMoveOnPlayerVsPlayer(event) {
    ...
    if (checkWin()) {
        ...
        textToPlay.textContent = `${(count % 2 === 0) ? 'O' : 'X'} to Play`;
...
function setResetGame() {
    ...
    handleMove = (isPlayerVsComputer) ? handleMoveOnComputerVsPlayer : handleMoveOnPlayerVsPlayer;
