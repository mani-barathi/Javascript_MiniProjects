...
async function getGithubUser(searchValue) {
  try {
    ...
    } else alert(`No user is available with name ${searchValue}`);
  } catch (error) {
    console.log(error);
  }
}
...
function renderUserProfile(userProfile) {
  let profileElementInnerHTML = `
    ...
    <div class="profile__info">
        ...
        <h5 class="profile__infoLogin">${
          userProfile.name ? userProfile.name : ""
        }</h5>
        <p class="profile__infoBio">${
        ...
        <div class="profile__infoStats">
            ...
            } <small class="profile__infoStats__Small">Repos</small>
            </span>

            <span class="profile__infoStatsObj">
            ${
              userProfile.followers
            } <small class="profile__infoStats__Small">Followers</small>
            </span>

            <span class="profile__infoStatsObj">
            ${
              userProfile.following
            } <small class="profile__infoStats__Small">Following</small>
            </span>            
        </div>
        <div class="profile__infoRepos"> </div>
        <a href="${
          userProfile.html_url
        }" target='_blank' class="profile__infoGitHubLink">Visit Github</a>
    `;
  ...
  if (userRepos.length > 0) {
    for (let i = 0; i < userRepos.length; i++) {
      for (let j = 0; j < userRepos.length - i - 1; j++) {
        const isFirstDateGreator =
          new Date(userRepos[j].updated_at.substring(0, 10)) <
          new Date(userRepos[j + 1].updated_at.substring(0, 10));
    ...
    const profileInfoReposElement = profileElement.querySelector(
      ".profile__infoRepos"
    );
    profileInfoReposElement.innerHTML = profileInfoReposInnerHTML;
  } else console.log("no repos");
}
